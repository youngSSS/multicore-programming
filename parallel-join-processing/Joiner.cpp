...
// Executes a join query
{
	...
	unordered_map<int, int> relationCounter;

	for (auto pInfo : query.predicates) {
		relationCounter[pInfo.left.relId] += 1;
		relationCounter[pInfo.right.relId] += 1;
	}

	for (auto pInfo : query.predicates) {
		if (relationCounter[pInfo.left.relId] == 1 && relationCounter[pInfo.right.relId] == 1) {
			auto left = addScan(usedRelations, pInfo.left, query);
			auto right = addScan(usedRelations, pInfo.right, query);
			unique_ptr<Operator> root = make_unique<Join>(move(left), move(right), pInfo);
	...
	// Predicate 연산 수행 전 Filter를 먼저 적용하여 탐색 범위를 줄이고 Predicate 연산을 수행한다.
